...
def gnuplot_command(gnuplot_variables,file_list,gen_list):
	...
	if catenate == False and _3D_mode == False:
		...
		for i in range(len(file_list)):
			...
			gnuplot_script_vars = "-e \"%s;data_file_name='%s';output_file_name='%s';header_length='%s';parser_data='%s';\"" % ("".join(var),file_list[i],outfilename,parser_output[0]," ".join(parser_output[1:]))
